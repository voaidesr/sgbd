...
\section*{Enunțul Problemei}
Sa se scrie o procedura stocata numita \texttt{raport\_financiar\_locatie} care primeste ca parametru numele unei locatii (oras). Procedura va afisa monumentele din acea locatie si sursele lor de finantare, folosind doua cursoare:

Cursorul Principal (Independent): Va itera prin toate monumentele care se afla in locatia specificata.

Cursorul Secundar (Parametrizat/Dependent): Pentru fiecare monument gasit de primul cursor, acest al doilea cursor va cauta toate sursele de finantare ale restaurarilor asociate acelui monument, primind ca parametru ID-ul monumentului.
